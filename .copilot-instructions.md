...
export async function getProductsData(categoryId?: string) {
  // Implementation
}
```
...
export async function GenericPageView({ slug }: GenericPageViewProps) {
...
```typescript
// src/shared/components/NewComponent/index.tsx
...
```typescript
// src/features/some-feature/actions/getSomeData.ts
...
### Dynamic Page
```typescript
// src/app/some-route/[slug]/page.tsx
interface PageProps {
  params: { slug: string };
}

export default async function Page({ params }: PageProps) {
  const data = await getSomeData(params.slug);
